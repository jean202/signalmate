...
flowchart TB
    ...
    subgraph API["API Layer"]
        ...
        ANAL["POST /conversations/:id/analyses"]
        GET_A["GET /analyses/:id"]
    end

    subgraph Engine["Analysis Engine Router"]
        direction TB
        ROUTER{"modelName?"}
        RULE["Rule-Based<br/>16 pattern rules"]
        HYBRID["Hybrid (Phase 1)<br/>Rules + Claude Chaining"]
        AGENT["Agent (Phase 4)<br/>Multi-step Tool Loop"]
    end
    ...
    subgraph AI["AI Pipeline"]
        ...
        RAG["RAG Context (Phase 3)<br/>pgvector similarity"]
    end

    subgraph AgentTools["Agent Tools (6)"]
        T1["analyze_timeline"]
        T2["detect_tone_shift"]
        T3["match_patterns"]
        T4["search_similar"]
        T5["check_quality"]
        T6["submit_result"]
    ...
    subgraph Data["Data Layer"]
        PG["PostgreSQL 16"]
        VEC["pgvector<br/>1536-dim embeddings"]
        EMBED["OpenAI<br/>text-embedding-3-small"]
    end

    UI --> CONV --> PG
    UI --> ANAL --> ROUTER
    ROUTER -->|rule-based-dev| RULE
    ROUTER -->|hybrid-v1| HYBRID
    ROUTER -->|agent-v1| AGENT
    ...
    style AGENT fill:#ffd700,stroke:#333
    style RAG fill:#98fb98,stroke:#333
```
...
POST   /api/v1/conversations/:id/analyses        # 분석 실행 (modelName 선택)
